...
| `draft_responsive_search_ad` | Create RSA preview (does NOT publish) | 3-15 headlines (≤30 chars), 2-4 descriptions (≤90 chars), final_url required |
...
7. Present the complete preview to the user — include any warnings from the pre-write checks
...
6. After campaign creation, remind the user to add ads via `draft_responsive_search_ad` and then enable the campaign via `enable_entity`
...
- Display path fields: **15 characters** each
